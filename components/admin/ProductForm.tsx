...
export default function ProductForm({ id }: { id?: string }) {
  ...
  return <form onSubmit={submit} className="editor-form">
    <section className="glass-card form-panel"><h2>Product details</h2>
      ...
      <div className="grid-3"><label className="form-group"><span className="form-label">Price</span><input className="form-input" required min="0" step="0.01" type="number" value={form.price} onChange={event => update('price', Number(event.target.value))}/>{form.discount > 0 && <small className="form-hint">Shown crossed out: ${form.price.toFixed(2)}</small>}</label><label className="form-group"><span className="form-label">Product discount (%)</span><input className="form-input" min="0" max="100" type="number" value={form.discount} onChange={event => update('discount', Number(event.target.value))}/>{form.discount > 0 && <small className="form-hint">Customer pays: ${salePrice.toFixed(2)}</small>}</label><label className="form-group"><span className="form-label">Stock</span><input className="form-input" min="0" type="number" value={form.stock} onChange={event => update('stock', Number(event.target.value))}/></label></div>
