...
export default async function ProductPage({ params }: { params: { storeSlug: string; id: string } }) {
  ...
  return <main className="store-section"><div className="product-detail"><div><ImageGallery images={images} alt={product.name}/></div><div className="product-info"><p className="eyebrow">{categoryName} · {store.name}</p><h1>{product.name}</h1><div className="product-detail-price">${salePrice.toFixed(2)} {referencePrice > salePrice && <del>${referencePrice.toFixed(2)}</del>}</div>{product.discount > 0 && <span className="product-discount-badge detail-discount">Save {product.discount}%</span>}<p className="product-description">{product.description}</p><div className="product-facts"><span><strong>SKU</strong>{product.sku || '—'}</span><span><strong>Category</strong>{categoryName}</span><span><strong>Availability</strong><b className={product.stock > 0 ? 'text-success' : 'text-error'}>{product.stock > 0 ? `${product.stock} in stock` : 'Out of stock'}</b></span></div>{product.tags?.length > 0 && <div className="product-tags">{product.tags.map(tag => <span key={tag}>{tag}</span>)}</div>}{properties.length > 0 && <section className="product-details-section"><h2>Product details</h2><dl className="properties">{properties.map((property, index) => <div key={`${property.key}-${index}`}><dt>{property.key}</dt><dd>{property.value}</dd></div>)}</dl></section>}<div className="product-assurance"><div><strong>Cash on delivery</strong><span>Pay securely when your order arrives.</span></div><div><strong>Quick dispatch</strong><span>We will contact you to confirm delivery.</span></div></div><AddToCart product={product} storeSlug={store.slug} storeName={store.name} whatsappNumber={store.whatsappNumber}/></div></div><section className="related-products"><div className="section-heading"><div><p className="eyebrow">YOU MAY ALSO LIKE</p><h2>Related products</h2><p className="text-secondary">More picks from {store.name} in the same collection.</p></div></div>{related.length > 0 ? <div className="products-grid">{related.map(item => <ProductCard key={item.id} product={item} slug={store.slug}/>)}</div> : <p className="empty-state">More products will appear here soon.</p>}</section></main>;
}
