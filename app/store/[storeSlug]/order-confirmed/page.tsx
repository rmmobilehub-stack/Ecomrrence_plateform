...
type SavedOrder = {
  ...
  whatsappNumber?: string; storeName?: string;
};
...
export default function ConfirmationPage({ params }: { params: { storeSlug: string } }) {
  ...
  const message = useMemo(() => {
    ...
    order.items.forEach((item, index) => { lines.push(`${index + 1}. ${item.productName} × ${item.qty} — $${(item.price * item.qty).toFixed(2)}`); const choices = Object.entries(item.selectedVariants ?? {}).map(([name, value]) => `${name}: ${value}`).join(', '); if (choices) lines.push(`   ${choices}`); });
    if (order.productDiscount) lines.push('', `Product discount: -$${order.productDiscount.toFixed(2)}`);
    if (order.couponCode) lines.push('', `Coupon: ${order.couponCode}`);
    if (order.discount) lines.push(`Discount: -$${order.discount.toFixed(2)}`);
    lines.push(`Delivery: ${order.deliveryFee ? `$${order.deliveryFee.toFixed(2)}` : 'Free'}`);
    if (order.customer.notes) lines.push(`Notes: ${order.customer.notes}`);
    lines.push('', `*Order total: $${order.total.toFixed(2)}*`, 'Payment: Cash on delivery');
    return lines.join('\n');
  }, [order, params.storeSlug]);

  return <main className="store-section"><div className="order-success"><div className="success-icon"><CheckCircle2 size={42}/></div><h1>Your order is confirmed!</h1><p className="text-secondary mt-2">We will contact you soon to confirm delivery.</p>{order && <div className="order-summary mt-4"><div className="order-summary-row"><span>Order number</span><strong>{order.orderNumber}</strong></div>{order.productDiscount ? <div className="order-summary-row text-success"><span>Product discount</span><span>−${order.productDiscount.toFixed(2)}</span></div> : null}{order.discount ? <div className="order-summary-row text-success"><span>Coupon discount</span><span>−${order.discount.toFixed(2)}</span></div> : null}<div className="order-summary-row"><span>Delivery</span><span className={order.deliveryFee ? '' : 'text-success'}>{order.deliveryFee ? `$${order.deliveryFee.toFixed(2)}` : 'Free 🎁'}</span></div><div className="order-summary-row total"><span>Total</span><span className="amount">${order.total.toFixed(2)}</span></div></div>}{order && <WhatsAppButton number={order.whatsappNumber} message={message} label="Send order to WhatsApp" className="confirmation-whatsapp"/>}<Link href={`/store/${params.storeSlug}/products`} className="btn btn-primary mt-4">Continue shopping</Link></div></main>;
}
