...
export default function CartPage({ params }: { params: { storeSlug: string } }) {
  ...
  if (!items.length) return <main className="store-section"><div className="page-header"><div><h1 className="page-title">Your cart</h1><p className="page-subtitle">Review your items before checkout.</p></div></div><div className="empty-state"><h3>Your cart is empty</h3><Link className="btn btn-primary mt-4" href={storefrontPath(params.storeSlug, 'products')}>Continue shopping</Link></div></main>;
