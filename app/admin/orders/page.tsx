...
export default function OrdersPage() {
  ...
  return <><div className="page-header"><div><h1 className="page-title">Orders</h1><p className="page-subtitle">Only {store?.name??'this store'}&apos;s website and WhatsApp orders appear here.</p></div></div><div className="filter-bar"><select className="form-select" value={status} onChange={event => setStatus(event.target.value)}><option value="">All statuses</option>{['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled'].map(value => <option key={value}>{value}</option>)}</select></div><div className="glass-card table-container"><table className="data-table"><thead><tr><th>Order</th><th>Source</th><th>Customer</th><th>Total</th><th>Status</th><th>Date</th><th/></tr></thead><tbody>{orders.length === 0 ? <tr><td colSpan={7}>No orders found.</td></tr> : orders.map(order => <tr key={order.id}><td className="font-semibold">{order.orderNumber}</td><td><span className={`badge badge-${order.channel === 'whatsapp' ? 'success' : 'info'}`}>{order.channel === 'whatsapp' ? 'WhatsApp' : 'Website'}</span></td><td><div>{order.customer.name}<small>{order.customer.email || (order.channel === 'whatsapp' ? 'Details pending in WhatsApp' : '')}</small></div></td><td>{formatMoney(order.total,store?.currency??'PKR')}</td><td><span className="badge badge-warning">{order.status}</span></td><td>{new Date(order.createdAt).toLocaleDateString()}</td><td><Link className="btn btn-ghost btn-sm" href={`/admin/orders/${order.id}`}>Open</Link></td></tr>)}</tbody></table></div></>;
}
