...
export async function POST(req: NextRequest) {
  ...
  await insertOne('discounts.json', discount); return NextResponse.json({ discount }, { status: 201 });
}

export async function PUT(req: NextRequest) {
  const session = await getSessionFromRequest(req);
  if (!session || session.role !== 'admin') return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const { id, isActive } = await req.json(); const discounts = await readDb<Discount>('discounts.json');
  ...
  const discount = await updateOne<Discount>('discounts.json', id, { isActive: Boolean(isActive) }); return NextResponse.json({ discount });
}

export async function DELETE(req: NextRequest) {
  const session = await getSessionFromRequest(req);
  if (!session || session.role !== 'admin') return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const id = new URL(req.url).searchParams.get('id'); const discounts = await readDb<Discount>('discounts.json');
  ...
  await deleteOne<Discount>('discounts.json', id); return NextResponse.json({ success: true });
}
