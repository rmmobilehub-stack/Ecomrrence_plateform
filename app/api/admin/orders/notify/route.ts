...
export async function POST(request: NextRequest) {
  ...
  const { orderId } = await request.json(); const orders = await readDb<Order>('orders'); const order = orders.find(entry => entry.id === orderId && entry.storeId === session.storeId);
  ...
  await insertOne('notifications', notification); return NextResponse.json({ notification }, { status: 201 });
}
