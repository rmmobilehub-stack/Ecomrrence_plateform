...
export async function GET(_: Request, { params }: { params: { slug: string; code: string } }) {
  const stores = await readDb<Store>('stores'); const store = stores.find(entry => entry.slug === params.slug && entry.isActive);
  if (!store) return NextResponse.json({ error: 'Store not found' }, { status: 404 });
  const discounts = await readDb<Discount>('discounts'); const discount = discounts.find(entry => entry.storeId === store.id && entry.code === params.code.toUpperCase() && entry.isActive && (!entry.expiresAt || new Date(entry.expiresAt) > new Date()));
